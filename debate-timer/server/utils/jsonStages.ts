...
export type StageType = 'speech' | 'question' | 'summary' | 'special' | 'dual-timer'
...
const dataFile = path.resolve(process.cwd(), 'server', 'data', 'stages.json')

async function ensureFile() {
  const dir = path.dirname(dataFile)
  try { await fs.mkdir(dir, { recursive: true }) } catch {}
  try { await fs.access(dataFile) } catch {
    await fs.writeFile(dataFile, JSON.stringify(DEFAULT_STAGES, null, 2), 'utf-8')
  }
}

export async function readStages(): Promise<Stage[]> {
  await ensureFile()
...
export async function writeStages(stages: Stage[]) {
  await ensureFile()
  // 兜底排序 & 整理 order
  stages.sort((a, b) => a.order - b.order)
  stages.forEach((s, i) => { s.order = i + 1 })
  await fs.writeFile(dataFile, JSON.stringify(stages, null, 2), 'utf-8')
}